...
**Prompt S/R** (v0.4.8 → v0.5.0): "Inherit main t2i prompt"가 ON일 때, 메인의 충돌 토큰(예: `shirt`)이 Refine의 새 prompt(`nude`)를 짓누르는 문제를 해결. Refine 패널의 S/R 시스템 (v0.5.0):

- **Auto-strip Mask Prompt tokens** 체크박스 (기본 ON) — Mask Prompt에 적은 토큰들이 자동으로 메인 prompt에서 segment 단위로 제거되고, Replacement Prompt가 그 자리에 **한 번만** 삽입됩니다. 예: Mask `white shirt, black necktie` + Replacement `nude` → 메인의 두 segment 모두 빠지고 `nude` 한 번 들어감.
- **Extra S/R 규칙** (선택) — 한 줄당 `pat1, pat2, ... = replacement` 문법. 멀티 패턴이 단일 replacement를 공유 (replacement는 첫 매치 위치에 1회만 삽입). 빈 replacement = 삭제.
- **Segment 단위 제거** — `shirt` 패턴이 "white shirt" segment 안에 매칭되면 전체 segment 제거 → 메인이 "white" 같은 고아 토큰으로 깨지지 않음.

LoRA·anatomy context(`2girls`, `grabbing pectorals` 등)는 그대로 유지되고 충돌 토큰만 빠지므로, 메인을 통째로 비우지 않고도 깨끗한 교체가 가능합니다.
