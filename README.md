...
5. **txt2img Workspaces** (v0.10.0+, v0.12.0 동적 관리) — 한 브라우저 탭 안에서 이름을 붙인 작업공간을 추가·삭제하고 프롬프트·생성 설정·스크립트·XYZ Plot 상태를 서로 독립적으로 전환
...
APG/CWM/SMC · DCW · DAVE · CNS · Adaptive Guidance · Detail Daemon)를 제공합니다.
...
## 워크플로 5: txt2img Workspaces (단일 탭 다중 작업공간) (v0.10.0+, v0.12.0 개선)

같은 `8760` 포트에서 기본 3개(최대 20개)의 브라우저 화면을 하나의 **Live Workspaces** 안에
유지합니다. 접속 시 확장 전용 경량 셸인 `/sam3-live`로 자동 이동하지만 포트와 Forge 서버는
그대로입니다. 각 Workspace는 별도 same-origin iframe의 Gradio 문서이므로 버튼을 누를 때 값을
갈아 끼우지 않고, 이미 설정된 화면의 표시만 즉시 전환합니다. 숨겨진 Workspace는 `inert` 상태라
클릭·단축키 입력을 받지 않으며 **Generate는 현재 보고 있는 Workspace 하나에서만 실행**됩니다.
...
더블클릭으로도 이름을 바꿀 수 있습니다.

각 화면은 위쪽 Prompt/Negative/Generate와 아래쪽 **Parameters / Scripts / Gallery** 3열로
정리됩니다. 좁은 화면에서는 한 열로 접힙니다. `기본 UI` 버튼은 기존 값 복원 방식 UI로 돌아가며,
그 메뉴에서 다시 Live Workspaces로 전환할 수 있습니다.
...
- 다른 포트·호스트·브라우저 프로필로 옮길 때는 작업공간 메뉴의 **내보내기/가져오기**를
  사용하세요.
- v0.12.0 업데이트 후에는 경량 셸의 Python 경로를 등록해야 하므로 브라우저 새로고침만 하지 말고
  **WebUI 프로세스를 한 번 완전히 재시작**하세요. `기본 UI` 버튼은 같은 포트의 기존 Forge UI로
  돌아갑니다.
...
|---|---|---|
| **PAG** | appended weak row만 hard value-only attention으로 교체 | 실제 Anima E2E 검증. Legacy Soft 토글 제공 |
| **SEG** | 실제 Anima T/H/W 중 H/W query에 Gaussian blur | 실제 Anima E2E 검증. Legacy uniform-value 근사 제공 |
...
`ADG/PAG 배치 → DAVE → attention PAG/SEG → CFG base(Preserve/APG/CWM/SMC/SMC+CWM)
→ PAG/SEG/SLG delta → DCW → CNS sampler noise`

- CFG base 기본은 **Preserve incoming**입니다. MaHiRo/custom CFG 결과를 그대로 유지합니다.
- 다른 CFG base는 incoming에서 `w_eff`를 복원해 Forge의 `edit_strength`를 보존하고, 비선형 fit
  오차가 크면 경고합니다.
- `Experimental stack`은 명시적으로 `SMC → APG → CWM`을 실행하는 고급 토글입니다.
