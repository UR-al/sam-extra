...
**Prompt S/R** (v0.4.8): "Inherit main t2i prompt"가 ON일 때, 메인의 충돌 토큰(예: `shirt`)이 Refine의 새 prompt(`nude`)를 짓누르는 문제를 해결. Refine 패널의 `Prompt S/R` 텍스트박스에 한 줄당 `pattern=replacement` 규칙을 적으면, **inherit 시점에 메인 prompt/negative에서 해당 토큰을 제거(빈 replacement)하거나 치환** 후 merge합니다. LoRA·anatomy context(`2girls`, `grabbing pectorals` 등)는 그대로 유지되고 충돌 토큰만 빠지므로, 메인을 통째로 비우지 않고도 깨끗한 override가 가능합니다.
