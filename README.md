...
## 별도 기능: Anima Perturbation Guidance (PAG / SEG / SLG) + APG (v0.9.8+)
...
감싸, 지정한 블록의 **약한 예측 행에만** 적용합니다. value는 RoPE가 없어 rotary-safe하며,
grouped-query로 head 수가 다르면 텐서를 훼손하지 않고 자동 스킵합니다.

### 세 가지 perturbation 방식 (v0.9.11+)

같은 골격 위에서 세 방식을 선택/조합합니다. 여러 개를 켜도 **약한 예측을 같은 배치에 접어**
단일 forward로 계산하며(활성 수만큼 배치 행만 증가), **PAG↔SEG는 성격이 겹쳐 택1, SLG는
병용 가능**합니다.
...
> SEG는 원논문의 가우시안 블러 대신 "uniform(∞-blur 극한=값 평균)으로 보간"하는 **shape-agnostic
> 근사**입니다(H·W 재구성 불필요, 안전). SLG는 SD3.5/Wan에서 정평난 레이어 스킵 방식입니다.

### 파라미터

| 필드 | 기본값 | 설명 |
|---|---|---|
| Enable Perturbation Guidance | off | 켜면 Anima 생성에만 적용 (다른 모델 자동 스킵) |
...
| **여러 perturbation 동시 시 scale 자동 감쇠** | on(**토글**) | 활성 수로 각 scale ÷ (과대 guidance 방지). 끄면 원 scale 그대로 |
...
쉽니다(그 지점은 PAG/SEG/SLG 이득이 거의 없음).
...
> perturbation의 배치-접기와 **정반대**(행 추가 ↔ 행 제거)이며, 같은 wrapper가 스텝별로
> 둘을 조율합니다. Detail Daemon·APG·MaHiRo와도 독립적으로 겹칩니다.

### XYZ Plot 비교

XYZ plot 축에 `[Anima Pert] …` 와 `[Anima APG] …` 항목이 추가됩니다. **`[Anima Pert] Enable`**
축을 `True, False`로 두면 **ON/OFF 비교 그리드**를 바로 뽑습니다(UI 체크박스와 무관하게 축
값이 우선). 그 외 `Attn Method / Attn Scale / Perturbation Strength / Attn Block Indices /
SLG Enable / SLG Scale / SLG Block Indices / Start·End Percent / Rescale` 축, 그리고
**`[Anima APG] Enable`**(ON/OFF) 및 `Eta / Norm Threshold / Momentum` 축, 그리고
**`[Anima AdaptiveG] Enable`**(ON/OFF) 및 `Skip After / Keep Every` 축도 제공됩니다.
ON 셀은 PNG 메타 `Anima Perturbation Guidance: …` / `Anima APG: …` /
`Anima Adaptive Guidance: …` 마커로 식별됩니다.
