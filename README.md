...
5. **txt2img Workspaces** (v0.10.0+) — 한 브라우저 탭 안에서 이름을 붙인 작업공간을 만들고 프롬프트·생성 설정·스크립트·XYZ Plot 상태를 서로 독립적으로 전환
...
## 워크플로 5: txt2img Workspaces (단일 탭 다중 작업공간) (v0.10.0+)

같은 `8760` 주소의 브라우저 탭 세 개를 하나의 **Live Workspaces** 화면 안에 유지합니다. 각
Workspace는 별도 same-origin iframe의 Gradio 문서이므로 버튼을 누를 때 값을 갈아 끼우지 않고,
이미 설정된 화면의 표시만 즉시 전환합니다. 숨겨진 Workspace는 `inert` 상태라 클릭·단축키 입력을
받지 않으며 **Generate는 현재 보고 있는 Workspace 하나에서만 실행**됩니다. Workspace 버튼은
더블클릭해 이름을 바꿀 수 있습니다.
...
- checkpoint/VAE 등 Forge 전역 **Quicksettings**는 작업공간에 포함하지 않습니다. 모델 재로딩을
  유발하는 전역 상태가 작업공간 전환과 섞이지 않도록 하기 위함입니다. iframe이 세 개여도 Forge
  서버·GPU·생성 큐와 현재 로드된 checkpoint는 하나이므로 이 항목들은 실제 브라우저 탭 3개처럼
  서로 공유됩니다.
