...
5. **txt2img Workspaces** (v0.10.0+) — 한 브라우저 탭 안에서 프롬프트·생성 설정·스크립트·XYZ Plot 상태가 서로 독립된 작업공간 3개를 즉시 전환
...
## 워크플로 5: txt2img Workspaces (단일 탭 작업공간 3개) (v0.10.0+)

같은 `8760` 주소의 브라우저 탭을 여러 개 열어 두는 대신, **Workspace 1 / 2 / 3** 버튼으로
txt2img 실험 세 개를 한 탭에서 전환합니다. 각 작업공간에는 다음 상태가 독립적으로 보존됩니다.
...
현재 작업공간은 입력을 바꿀 때마다 **브라우저 로컬 저장소에 자동 저장**됩니다. 아직 저장 내용이
없는 작업공간을 처음 열면 현재 설정을 그대로 복제하므로, 공통 설정을 다시 입력한 뒤 차이만
수정할 필요가 없습니다. Forge Neo 기본 파일은 수정하지 않으며 이 확장의 프런트엔드에서만
동작합니다.

### 저장 범위와 주의사항

- 이미지·파일 입력, 갤러리, 생성 결과/output은 저장하거나 복원하지 않습니다.
- checkpoint/VAE 등 Forge 전역 **Quicksettings**는 작업공간에 포함하지 않습니다. 모델 재로딩을
  유발하는 전역 상태가 작업공간 전환과 섞이지 않도록 하기 위함입니다.
